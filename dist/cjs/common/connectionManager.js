...
class ConnectionStateConnected {
    constructor(serviceProvider, connectionStringAuthType, connectedAtlasCluster) {
        this.serviceProvider = serviceProvider;
        this.connectionStringAuthType = connectionStringAuthType;
...
class MCPConnectionManager extends ConnectionManager {
    ...
    async connect(settings) {
        ...
        try {
            ...
            connectionStringAuthType = MCPConnectionManager.inferConnectionTypeFromSettings(this.userConfig, connectionInfo);
        ...
        try {
            if (connectionStringAuthType.startsWith("oidc")) {
                return this.changeState("connection-request", {
                    ...
                    connectedAtlasCluster: settings.atlas,
                    connectionStringAuthType,
                    oidcConnectionType: connectionStringAuthType,
                });
            }
            return this.changeState("connection-success", new ConnectionStateConnected(await serviceProvider, connectionStringAuthType, settings.atlas));
        }
        catch (error) {
            const errorReason = error instanceof Error ? error.message : `${error}`;
            this.changeState("connection-error", {
    ...
    async onOidcAuthSucceeded() {
        if (this.currentConnectionState.tag === "connecting" &&
            this.currentConnectionState.connectionStringAuthType?.startsWith("oidc")) {
            this.changeState("connection-success", new ConnectionStateConnected(await this.currentConnectionState.serviceProvider, this.currentConnectionState.connectionStringAuthType, this.currentConnectionState.connectedAtlasCluster));
        }
        this.logger.info({
    ...
    static inferConnectionTypeFromSettings(config, settings) {
        const connString = new mongodb_connection_string_url_1.ConnectionString(settings.connectionString);
