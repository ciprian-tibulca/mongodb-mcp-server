...
export class GetPerformanceAdvisorTool extends AtlasToolBase {
    constructor() {
        ...
        this.description = `Get MongoDB Atlas performance advisor recommendations, which includes the operations: suggested indexes, drop index suggestions, schema suggestions, and a sample of the most recent (max ${DEFAULT_SLOW_QUERY_LOGS_LIMIT}) slow query logs`;
        this.argsShape = {
        ...
        try {
            const [suggestedIndexesResult, dropIndexSuggestionsResult, slowQueryLogsResult, schemaSuggestionsResult] = await Promise.allSettled([
                ...
                operations.includes("slowQueryLogs")
                    ? getSlowQueries(this.session.apiClient, projectId, clusterName, since ? new Date(since) : undefined, namespaces)
                    : Promise.resolve(undefined),
